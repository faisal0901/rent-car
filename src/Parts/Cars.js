  ...
  return (
    <div className="flex justify-start mt-10 flex-wrap">
      {data.map((val, index) => {
        return (
          <div
            className="w-1/3 px-4 mb-6 shadow-lg rounded"
            key={`car-${index}`}
          >
            ...
            <div className="justify-between flex  items-center border-b-2 border-gray-300">
              ...
              <div className="flex-wrap w-2/3 justify-end">
                {val.feature.map((val2, index2) => (
                  <div
                    className="icon-1 inline-flex mt-2 mr-2 items-center  mb-2"
                    key={`feature-${index2}`}
                  >
                    <img src={val2.featureImage} alt={val2.featureQuantity} />
